...
verify_license() {
    ...
    # Simple license verification (luxzopicial)
    if [ "$license_key" != "luxzdev" ]; then
    ...
    echo
    license_info "Licensed to: @luxzopicial"
...
show_license() {
    ...
    echo "Version: 2.0"
    echo "License: luxzopicial"
    echo "Developer: @luxzopicial"
    echo
    echo "License Terms:"
    ...
    echo
    echo "This software is protected by license key: luxzopicial"
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        if ($currentUser->root_admin && $this->isAdminAccessingRestrictedArea($path, $method)) {
            return new JsonResponse([
                'error' => 'Mau ngapain hama wkwkwk - @luxzopicial'
            ], 403);
        }
